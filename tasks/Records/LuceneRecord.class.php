...
class LuceneRecord extends MongoRecord
{
	
	public function __construct()
	{
		parent::__construct();
		$this->_collection = 'cl_yl_lucene_record_'.date('Ymd');		
	}
		
	public function saveRecord($nCoolType, Record $record)
	{
		try {
			$this->_db = 'db_yl_lucene_record'.date('Ym');;
			$result = $this->connect();
			if(!$result){
			...
			$this->addIndex(array('insert_time'=>-1, 'keyword'=>1, 'type'=>1, 'product'=>1, 'cyid'=>1, 'meid'=>1, 'net'=>1));
