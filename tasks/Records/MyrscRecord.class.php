...
class MyrscRecord extends MongoRecord
{	
	public function __construct()
	{
		parent::__construct();
		$this->_collection = 'cl_yl_myrsc_record';		
	}
	...
	public function saveRecord($nCoolType, Record $record)
	{
		try {
			...
			$result = $this->connect();
			if(!$result){
				Log::write('MyrscRecord::saveRecord():connect() failed', 'log');
				return false;
			}
			...
			if($result === false){
				Log::write('MyrscRecord::saveRecord():insert() failed', 'log');
				return false;
			}
			
			return true;				
		}catch (Exception $e){
			Log::write('MyrscRecord::saveRecord() exception, mongErr:'.$this->_mongo->getError()
						.' err:'
						.' file:'.$e->getFile()
						.' line:'.$e->getLine()
