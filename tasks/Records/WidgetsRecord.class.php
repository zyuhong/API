...
class WidgetsRecord extends MongoRecord
{
	...
	public function saveRecord($nCoolType, Record $record)
	{
		try {
			...
			$result = $this->connect();
			if(!$result){
				Log::write('WidgetRecord::saveRecord():connect() failed', 'log');
				return false;
			}
			...
			if($result === false){
				Log::write('WidgetRecord::saveRecord():insert() failed', 'log');
				return false;
			}
			
			return true;				
		}catch (Exception $e){
			Log::write('WidgetRecord::saveRecord() exception, mongErr:'.$this->_mongo->getError()
						.' err:'
						.' file:'.$e->getFile()
						.' line:'.$e->getLine()
